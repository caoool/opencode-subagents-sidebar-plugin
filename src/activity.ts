...
export function normalizeMeaningful(value: unknown): string | undefined {
...
export function activityForPart(part: Part): string | undefined {
  if (part.type === "text" || part.type === "reasoning") return normalizeMeaningful(part.text)
  ...
  if (part.state.status === "pending") return `${name}…`
